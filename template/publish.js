    ...
    if (items.length) {
        ...
        items.forEach(function(item) {
            if ( !hasOwnProp.call(item, 'longname') ) {
                ...
                var itemsLink = linktoFn(item.longname, displayName.replace(/\b(module|event):/g, ''));
                itemsNav += "\t\t\t\t\t\t\t" + '<li class="here-2-' + item.longname + '">' + itemsLink + '</li>' + "\n\t";
        ...
        if (itemsNav !== '') {
            nav += "\t\t\t\t" + '<li class="nav-item-' + itemHeading.replace(/<[^>]*>?/gm, '').toLowerCase() + '">' + "\n\t\t\t\t\t\t" + '<span class="sub-item">' + itemHeading + '</span>' + "\n\t\t\t\t\t\t" + '<ul>' + "\n" + itemsNav + "\t\t\t\t\t" + '</ul>' + "\n\t\t\t\t\t" + '</li>' + "\n\t";
        }
    }
